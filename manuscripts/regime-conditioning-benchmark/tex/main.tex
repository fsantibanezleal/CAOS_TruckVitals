...
\newcommand{\versiondoi}{10.5281/zenodo.22824879}
...
Hotelling $T^2$, $0.06 \to 1.00$), leaves the detection rate of six unchanged (four at ceiling, SPE at
...
($0.69 \to 0.06$) and the one-class SVM ($0.75 \to 0.50$ on detection rate; its shorter conditioned delay
...
are almost no gaps to blame. Other learned rungs are helped: the dense autoencoder ($0.75 \to 1.00$,
$174 \to 90$ minutes) and Deep SVDD ($0.81 \to 0.94$), while the LSTM encoder-decoder is at ceiling on both
arms.
...
The measurements stand: the isolation forest ($0.688 \to 0.062$) and the one-class SVM ($0.750 \to 0.500$)
